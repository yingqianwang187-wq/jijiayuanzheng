...
# ------------------------------------------------------------------
# 指挥官等级（设计文档 §4.7，v0.18；数值为推荐值【待确认】）
#   经验来源：推关 / 任务领奖 / 成就领奖；每 5 级一次性 +10 召唤券
# ------------------------------------------------------------------
const COMMANDER_EXP_PER_LEVEL := 100          # 升下一级所需经验（固定每级 100）
const COMMANDER_TEN_TICKETS := 10              # 每 5 级送 10 召唤券（= 1 次免费十连）
const COMMANDER_TEN_EVERY_LEVELS := 5
const COMMANDER_EXP_STORY := 10                # 推主线 1 关 +10
const COMMANDER_EXP_TASK := 5                  # 任务领奖 1 次 +5
const COMMANDER_EXP_ACHIEVEMENT := 20          # 成就领奖 1 次 +20

# ------------------------------------------------------------------
# 新手引导（设计文档 §10.11，v0.18：6 步核心循环；step 0~6，6 = 完成）
# ------------------------------------------------------------------
const GUIDE_STEPS := [
	{ "name": "点击收获", "desc": "点击右上角「收获」按钮领取挂机金币与经验" },
	{ "name": "升级机娘", "desc": "在机娘列表点击「升级」提升战力" },
	{ "name": "推主线", "desc": "进入主线挑战第 1 关" },
	{ "name": "抽卡", "desc": "在抽卡界面进行一次召唤（新手十连免费）" },
	{ "name": "布阵", "desc": "在布阵界面调整上阵机娘" },
	{ "name": "秘境扫荡", "desc": "通关秘境后使用扫荡快速获取资源" },
]

# ------------------------------------------------------------------
# 活动（设计文档 §10.3 活动福利，v0.18：本地限时任务，本地规则驱动）
#   condition.type 复用成就进度类型；达成领奖一次性
# ------------------------------------------------------------------
const ACTIVITIES := {
	&"act_story3": { "name": "主线推进", "desc": "通关主线 3 关", "condition": { "type": "story_cleared", "target": 3 }, "reward": [ { "type": "diamond", "amount": 100 } ] },
	&"act_summon5": { "name": "召唤达人", "desc": "累计抽卡 5 次", "condition": { "type": "summon_count", "target": 5 }, "reward": [ { "type": "ticket", "amount": 1 } ] },
	&"act_tower10": { "name": "爬塔试炼", "desc": "爬塔 10 层", "condition": { "type": "tower", "target": 10 }, "reward": [ { "type": "gem", "quality": "white", "amount": 1 } ] },
}
...
const STORY_LINES := {
	...
	2: { "opening": "阿岚：呼……麻烦死了。小钰你冲那么快干嘛。", "clear": "还行，没拖后腿。" },
